...
public ArrayList<BoardComment> getBoardList(int commentNo) {
		
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;//조회 결과에 접근하는 참조 변수
		//데이터베이스의 데이터를 읽어서 저장할 객체 컬렉션
		ArrayList<BoardComment> coment = new ArrayList<BoardComment>();
		
		try {
			conn = ConnectionHelper.getConnection("oracle");
			StringBuffer sql = new StringBuffer(500);
			sql.append("select * from BoardComment");
			...
			while (rs.next()) {
				...
				co.setCommentNo(rs.getInt(1));
				co.setBoardNo(rs.getInt(2));
				co.setWriter(rs.getString(3));
				co.setRegDate(rs.getDate(4));
	...
	public void insertComment(BoardComment co) {
		Connection conn = null;		
		PreparedStatement pstmt = null;
		int count = 0;
		try {
			...
			//댓글 삽입
		String sql = "insert into BoardComment (commentNo, BOARDNO, writer, content,regdate) values\r\n" + 
				"(BoardComment_sequence.nextval, 1, 'gim','으아아',sysdate)";			
				
			pstmt = conn.prepareStatement(sql.toString());
			pstmt.setInt(1, co.getCommentNo());//제목
			pstmt.setInt(2, co.getBoardNo());//작성자
			pstmt.setString(3, co.getWriter());//내용
			pstmt.setString(4, co.getContent());//step
